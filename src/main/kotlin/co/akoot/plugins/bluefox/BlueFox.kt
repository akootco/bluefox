...
import co.akoot.plugins.bluefox.commands.*
...
class BlueFox : FoxPlugin("bluefox") {
    ...
    companion object {
        ...
        val onlinePlayerNames get() = server.onlinePlayers.map { it.name }
        ...
        fun query(@Language("SQL") sql: String, vararg autoGeneratedKeys: Int): PreparedStatement {
            if (!this::db.isInitialized) throw Exception("Database is not connected!")
            return db.prepareStatement(sql.trimIndent(), autoGeneratedKeys)
        }

        fun getOfflinePlayer(uuid: UUID): OfflinePlayer? {
            return server.getOfflinePlayer(uuid).takeIf { it.hasPlayedBefore() }
        }
    }
    ...
    override fun load() {
        ...
        try {
            ...
            println("Could not connect to market database!")
        }
