...
class PayCommand(plugin: BlueFox) : CatCommand(plugin, "pay", "Pay players from anywhere!") {
    ...
    fun pay(sender: CommandSender, player: OfflinePlayer?, amount: Double, currency: String = "AKC"): Boolean {
        ...
        val wallet =
            player.wallet ?: return sender.sendWarning("It appears ", player, " does not have a wallet. Sucks!")
        val coin = Market.getCoin(currency) ?: return sender.sendWarning(
            "There isn't a coin with the name \"",
            accent(currency),
            "\"."
        )
        ...
        return sender.sendText(
            ...
            player,
            " ",
            tertiary(amount.asCurrency),
            " ",
            accent(currency.uppercase()),
            "!"
        )
    }
}