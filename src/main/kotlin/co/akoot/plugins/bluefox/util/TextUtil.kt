...
fun MutableList<Component>.join(
    separator: Component?,
    tint: TextColor? = null,
    mutation: (Component) -> Component = { it }
): Component {
...
fun CommandSender.sendList(
    components: MutableList<Component>,
    separator: Component = Component.newline(),
    color: TextColor? = null,
    mutator: (Component) -> Component = { it }
) {
...
operator fun Component.div(component: Component): MutableList<Component> =
    mutableListOf(this, Component.newline(), component)
...
fun Component.strikethrough(strikethrough: Boolean = true): Component =
    this.decoration(TextDecoration.STRIKETHROUGH, strikethrough)
...
fun head(offlinePlayer: OfflinePlayer): Component =
    Component.`object`(ObjectContents.playerHead(offlinePlayer.uniqueId))
...
val colorCodes: Map<String, TextColor>
    get() = mapOf(
...
val palettes: Map<String, List<TextColor>>
    get() = mapOf(
...
fun getColor(string: String): TextColor? {
    return colorCodes[string] ?: palettes[string.dropLast(1)]?.getOrNull(string.last().digitToInt())
...
private fun applyColorCode(token: Token.ColorCode, state: FormatState): FormatState {
    ...
    var bold = state.bold;
    var italic = state.italic
    var underline = state.underline;
    var strike = state.strikethrough;
